...
test("start", () => {
    ...
    expect(state.getTargetDeckId()).toBe(123n);
});
...
test("concept scheduler deck flag is saved", () => {
    const state = startingState();
    const limits = get(state.deckLimits);
    limits.conceptSchedulerEnabled = true;

    const out = state.dataForSaving(UpdateDeckConfigsMode.NORMAL);

    expect(out.limits?.conceptSchedulerEnabled).toBe(true);
});

test("concept scheduler deck flag defaults to disabled", () => {
    const state = startingState();

    expect(get(state.deckLimits).conceptSchedulerEnabled).toBe(false);
});

test("concept scheduler deck flag can be saved as disabled", () => {
    const state = startingState();
    const limits = get(state.deckLimits);
    limits.conceptSchedulerEnabled = true;
    limits.conceptSchedulerEnabled = false;

    const out = state.dataForSaving(UpdateDeckConfigsMode.NORMAL);

    expect(out.limits?.conceptSchedulerEnabled).toBe(false);
});
