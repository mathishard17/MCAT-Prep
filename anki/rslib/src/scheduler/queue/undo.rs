...
use super::ConceptQueueSnapshot;
...
pub(crate) struct QueueUpdate {
    ...
    pub concept_before: Option<ConceptQueueSnapshot>,
    pub concept_after: Option<ConceptQueueSnapshot>,
}
        ...
        match change {
            UndoableQueueChange::CardAnswered(update) => {
                if let Some(queues) = self.get_or_invalidate_queues(update.queue_build_time)? {
                    ...
                    if let Some(snapshot) = update.concept_before.clone() {
                        queues.restore_concept_queue_snapshot(snapshot);
                    } else {
            ...
            UndoableQueueChange::CardAnswerUndone(update) => {
                if let Some(queues) = self.get_or_invalidate_queues(update.queue_build_time)? {
                    ...
                    queues.restore_cutoff(&update.cutoff_snapshot);
                    if let Some(snapshot) = update.concept_after.clone() {
                        queues.restore_concept_queue_snapshot(snapshot);
                    }
                }
