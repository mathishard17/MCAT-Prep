...
impl Collection {
    ...
    pub(crate) fn answer_card_inner(&mut self, answer: &mut CardAnswer) -> Result<()> {
        ...
        let concept_home_deck = updater.original_deck.clone();
...
pub(crate) mod test {
    ...
    #[test]
    fn answer_ratings_map_to_concept_evidence() {
        assert_eq!(evidence_from_rating(Rating::Again), Evidence::Negative);
        assert_eq!(evidence_from_rating(Rating::Hard), Evidence::Negative);
        assert_eq!(evidence_from_rating(Rating::Good), Evidence::Positive);
        assert_eq!(evidence_from_rating(Rating::Easy), Evidence::Positive);
    }

    #[test]
    fn concept_mastery_updates_on_answer() -> Result<()> {
        let mut col = Collection::new();
        let deck_id = DeckId(1);
        enable_concept_scheduler(&mut col, deck_id)?;
        add_basic_note_with_tags(&mut col, deck_id, &["KC::Bio::DNA"])?;
        ...
        assert_eq!(dna.negative, 0);
        assert!(dna.mastery > persisted.config.initial_mastery);
        assert_eq!(persisted.state.total_seen_cards, 1);
        let today = col.timing_today()?.days_elapsed;
        assert_eq!(persisted.state.daily.get(&today).unwrap().positive, 1);
        assert_eq!(persisted.state.prerequisite_violations, 0);

        Ok(())
    }

    #[test]
    fn concept_overview_cards_do_not_update_mastery_on_answer() -> Result<()> {
        let mut col = Collection::new();
        let deck_id = DeckId(1);
        enable_concept_scheduler(&mut col, deck_id)?;
        add_basic_note_with_tags(
            &mut col,
            deck_id,
            &["KC::Bio::DNA", "Overview::Bio::DNA"],
        )?;

        col.answer_good();

        let persisted = col.get_concept_scheduler_state(deck_id);
        assert!(!persisted.state.kcs.contains_key(&kc("Bio::DNA")));
        assert_eq!(persisted.state.total_seen_cards, 0);
        assert!(persisted.state.daily.is_empty());

        Ok(())
    }

    #[test]
    fn weak_prerequisites_are_counted_as_concept_violations() -> Result<()> {
        let mut col = Collection::new();
        let deck_id = DeckId(1);
        enable_concept_scheduler(&mut col, deck_id)?;
        add_basic_note_with_tags(
        ...
        let persisted = col.get_concept_scheduler_state(deck_id);
        let today = col.timing_today()?.days_elapsed;
        assert_eq!(persisted.state.prerequisite_violations, 1);
        assert_eq!(
            persisted
                .state
                .daily
    ...
    #[test]
    fn disabled_concept_scheduler_does_not_update_on_answer() -> Result<()> {
        let mut col = Collection::new();
        let deck_id = DeckId(1);
        add_basic_note_with_tags(&mut col, deck_id, &["KC::Bio::DNA"])?;
    ...
    #[test]
    fn untagged_cards_do_not_update_concept_state() -> Result<()> {
        let mut col = Collection::new();
        let deck_id = DeckId(1);
        enable_concept_scheduler(&mut col, deck_id)?;
        add_basic_note_with_tags(&mut col, deck_id, &[])?;
    ...
    #[test]
    fn undo_restores_concept_mastery_update() -> Result<()> {
        let mut col = Collection::new();
        let deck_id = DeckId(1);
        enable_concept_scheduler(&mut col, deck_id)?;
        add_basic_note_with_tags(&mut col, deck_id, &["KC::Bio::DNA"])?;
        ...
        col.answer_good();
        assert_eq!(
            col.get_concept_scheduler_state(deck_id)
                .state
                .total_seen_cards,
            1
        );
