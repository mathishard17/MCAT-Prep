...
class AddCards(QMainWindow):
    ...
    def setupEditor(self) -> None:
        ...
        self.editor.on_concept_metadata_changed = self._update_add_button_enabled
    ...
    def _update_add_button_enabled(self) -> None:
        if hasattr(self, "addButton"):
            # Keep the button clickable so manual tag edits can be validated on click
            # after the editor has flushed its current field/tag state.
            self.addButton.setEnabled(self.editor.note is not None)
    ...
    def _add_current_note(self) -> None:
        ...
        if not self.editor.concept_topic_selected():
            tooltip("Choose target KC and difficulty tags first.")
            return
        self.editor._apply_concept_metadata_tags()
