...
"""Stdlib-only core for the MCAT AI features (Track H).

This module holds the OpenAI client, key/model resolution, `.env` reading, and
the prompt strings — everything that talks to OpenAI without needing Qt or the
rest of the ``aqt`` package. Keeping it dependency-free (only ``json``, ``os``,
``re``, ``urllib``, ``pathlib``, ``typing``) means it can be imported and unit
tested on its own (e.g. the offline degradation test) without pulling in Qt.

``aqt.mcat_ai`` re-exports every public name here, so existing imports such as
``from aqt.mcat_ai import OpenAIClient`` keep working unchanged.
"""
