...
import com.ichi2.anki.conceptscheduler.McatTopics
import com.ichi2.anki.conceptscheduler.buildConceptTags
...
class NoteEditorFragment :
    ...
    private fun setupConceptSchedulerPanel() {
        ...
        toggle.setOnClickListener {
            val panel = conceptSchedulerPanel ?: return@setOnClickListener
            val show = panel.visibility != View.VISIBLE
            panel.visibility = if (show) View.VISIBLE else View.GONE
            toggle.setText(
                if (show) R.string.concept_scheduler_tags_hide else R.string.concept_scheduler_tags_show,
            )
        }
    }
    ...
    /** True when the current tag set has both a `KC::` and a `Difficulty::` tag (demo requirement). */
    private fun hasRequiredConceptTags(): Boolean {
        val tags = selectedTags?.map { normalizeConceptTag(it) } ?: return false
        return tags.any { it.startsWith("KC::") } && tags.any { it.startsWith("Difficulty::") }
    }
    ...
    private fun resetConceptSchedulerPanel() {
        ...
        conceptSchedulerPanel?.visibility = View.GONE
        requireView().findViewById<android.widget.Button>(R.id.concept_scheduler_toggle)
            .setText(R.string.concept_scheduler_tags_show)
    }
